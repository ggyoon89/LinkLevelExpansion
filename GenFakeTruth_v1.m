function [TrueUncorrPair,TrueCorrPairMean,TrueCorrPairCov,FlowCluster,PrUncorrPair,PrCorrPairMean]=GenFakeTruth(PUMAinfo,nPairSignif,NodeCluster,level)
%{
[OUTPUTS]
...
for i=1:nPair
    if P(i,4)>0
        if level==1
            maxdev=((6-log10(P(i,4)))*2+6)/100; mindev=0.05; % low
        elseif level==2
            maxdev=((6-log10(P(i,4)))*2+15)/100; mindev=0.05; % mid
        else
            maxdev=((6-log10(P(i,4)))*2+25)/100; mindev=0.05; % high
        end
        %{
...
% Separate flows 
PrCorrPairMean=zeros(1,3);  % [FlowID, mean, std.dev]
PrUncorrPair=zeros(1,3);    % [FlowID, mean, std.dev]
m=0;n=0;
for i=1:nPair
    if ismember(i,CorrelatedPairs)
        m=m+1;
        PrCorrPairMean(m,:)=P(i,[1,4,5]);
    else
        n=n+1;
        PrUncorrPair(n,:)=P(i,[1,4,5]);
    end
end
nCorrPair=m; nUncorrPair=n;
%% Generate fake truth (covariance, std.dev, mean)
% Uncorrelated flow: norminv
TrueUncorrPair=PrUncorrPair;
PrUncorrPair(:,3)=PrUncorrPair(:,2)*0.05; % assume prior std.dev as 5% of prior mean
TrueUncorrPair(:,2)=norminv(rand(nUncorrPair,1),PrUncorrPair(:,2),TrueUncorrPair(:,3));
for i=1:nUncorrPair
...
% TrueUncorrFlow: [FlowID, mean, std.dev] in FlowID order
Q=PrCorrPairMean;
%% Correlated flow
...
D=zeros(nCorrPair);
for i=1:nCorrPair
    D(i,i)=PrCorrPairMean(i,3); % assign std.dev to diagonal of D    
end
TrueCorrPairCov=D*TrueCorrMat*D;   % derive covariance matrix
PP=eig(TrueCorrPairCov);
sum(PP<0) 

% generate truth 
TrueCorrPairMean=[PrCorrPairMean(:,1),mvnrnd(PrCorrPairMean(:,2),TrueCorrPairCov)',PrCorrPairMean(:,3)];
