...
function [mu_est, OC, choices, ObsOD, PrUncorr, PrCorrMean,PrecisU,PrecisC]=Example_kg_transfer_20220801(mu,mu_0,sigma,sigma_0,M,ObsOD,CoveredOD,ODSysNet,PrUncorr,TrUncorr,PrCorrMean,TrCorrMean,TrCorrCov,PrecisU,PrecisC)


K=length(mu_0); %number of available choices

mu_est=mu_0;        % estimated mu from prior
sigma_est=sigma_0;	% estimated sigma from prior
...
%                 CovM(i,j)=CovM(i,j)+CovOD(CoveredOD{i}(m),CoveredOD{j}(n));
%             end
%         end
%         if i~=j
%             CovM(j,i)=CovM(i,j);
...
%     for i=1:nCoverOD
%         if ismember(TotalOD(i),CoveredOD{x})
%             coveredODInd(i,1)=1;
%         end
%     end
    for i=1:nCoverOD
...
%         subOD1=CoveredOD{x}(i);
%         subOD1=TotalOD(i);
%         for j=1:nCoverOD
% %             subOD2=CoveredOD{x}(j);
%             subOD2=TotalOD(j);
%             CovExtract(i,j)=TrueODCov(subOD1,subOD2);
%         end
%         MeanExtract(i,1)=TrueODMean(CoveredOD{x}(i),1);
%         MeanExtract(i,1)=TrueODMean(TotalOD(i),1);
%         StdevExtract(i,1)=TrueODStdev(TotalOD(i),1);
%     end
%     W=mvnrnd(MeanExtract',CovExtract);
%     W=normrnd(MeanExtract',StdevExtract');
%     W=normrnd(MeanExtract',StdevExtract');
    
    % Replace observation in W if correlated
%     InvolvedFlow=intersect(TotalOD,CorFlow(:,2)');
...
%             InterOD(i,j)=1;
%             PriorODMean(i,1)=(PriorODMean(i,1)*PriorODMean(i,2)+sum(ObsOD{i,3+j}))/(PriorODMean(i,2)+size(ObsOD{i,3+j},2));
%             PriorODMean(i,2)=PriorODMean(i,2)+size(ObsOD{i,3+j},2);
%         end
%     end
% end
